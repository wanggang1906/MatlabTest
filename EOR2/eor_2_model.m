%deal with
function [Portfolio]=eor_2_model(temp_insample,temp_oursample)
    Temp = temp_insample;  %initialize the worksapce(Insample) change the variable to calculate different cases
    M = 1e7;%M is an abitry large number
    ...
    Mean_security_rate = mean(Temp');  %this is rate of each security
    Mean_week_rate = mean(Temp);        
    Portfolio = zeros(n,length(alpha));
    for i=1:length(alpha)
        Alpha =alpha(i);
    benchAlpha = Benchmark +Alpha*ones(1,T);
    ...
    A(1,n+1) = 1;
    A(2:n+1,1:n) = -eye(n,n);
    A(n+2:n+T+1,n+1) =Mean_week_rate'+Alpha*ones(T,1);
